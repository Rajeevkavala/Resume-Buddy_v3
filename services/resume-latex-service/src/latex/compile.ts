...
function truncate(s: string, max = 16_000): string {
  if (s.length <= max) return s;
  return s.slice(0, max) + '\n...[truncated]';
}

export async function compileWithTectonic(latexSource: string, opts?: { timeoutMs?: number }): Promise<CompileResult> {
  ...
  try {
    ...
    const { code, stdout, stderr, timedOut, spawnError } = await new Promise<{
      code: number | null;
      stdout: string;
      stderr: string;
      timedOut: boolean;
    ...
    if (spawnError) {
      ...
      (err as any).code = code === 'ENOENT' ? 'TECTONIC_NOT_FOUND' : 'TECTONIC_SPAWN_FAILED';
      (err as any).details = {
        code,
        message: e?.message,
      };
    ...
    if (code !== 0) {
      ...
      (err as any).code = 'TECTONIC_FAILED';
      throw err;
    }
