...
export class DeploymentWorker extends BaseMonitoringWorker {
  ...
  protected async runProbe(
    ...
    const token = process.env.V_API_TOKEN || process.env.VERCEL_API_TOKEN || process.env.MONITOR_VERCEL_TOKEN || process.env.VERCEL_TOKEN;
    const projectId = process.env.V_TARGET_PROJECT_ID || process.env.VERCEL_TARGET_PROJECT_ID || process.env.VERCEL_PROJECT_ID;
